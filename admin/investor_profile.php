...
$title = 'Investor Profile - ' . $investor['name'];
require_once 'includes/header.php';
require_once 'includes/sidebar.php';

$inv_balance = getInvestorBalance($id);
$profit_balance = getInvestorProfitBalance($id);

$r = $db->getdata("SELECT COALESCE(SUM(amount),0) as tot FROM investor_ledger WHERE investor_id = $id AND type = 'deposit'");
$total_deposit = $r[0]['tot'] ?? 0;
$r = $db->getdata("SELECT COALESCE(SUM(amount),0) as tot FROM investor_ledger WHERE investor_id = $id AND type = 'investment_withdraw'");
$total_inv_withdraw = $r[0]['tot'] ?? 0;
$r = $db->getdata("SELECT COALESCE(SUM(amount),0) as tot FROM investor_ledger WHERE investor_id = $id AND type = 'profit'");
$total_profit = $r[0]['tot'] ?? 0;
$r = $db->getdata("SELECT COALESCE(SUM(amount),0) as tot FROM investor_ledger WHERE investor_id = $id AND type = 'profit_withdraw'");
...
<div class="flex items-center justify-between mb-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900"><?= htmlspecialchars($investor['name']) ?></h1>
        <span class="text-xs bg-gray-100 text-gray-500 px-3 py-1 rounded-full font-medium">#<?= $id ?></span>
    </div>
    <a href="<?= domain ?>admin/investor_edit.php?id=<?= $id ?>" class="inline-flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-xl text-sm font-medium hover:bg-gray-50 transition">
        <i class="bi bi-pencil"></i> Edit
    </a>
</div>

<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm">
        <span class="text-xs text-gray-400 uppercase tracking-wide">Phone</span>
        <p class="text-sm font-medium text-gray-800 mt-0.5"><?= htmlspecialchars($investor['phone']) ?></p>
    </div>
    <div class="bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm">
        <span class="text-xs text-gray-400 uppercase tracking-wide">Email</span>
        <p class="text-sm font-medium text-gray-800 mt-0.5"><?= htmlspecialchars($investor['email']) ?: '—' ?></p>
    </div>
    <div class="bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm">
        <span class="text-xs text-gray-400 uppercase tracking-wide">Admin Share</span>
        <p class="text-sm font-medium text-gray-800 mt-0.5"><?= $investor['profit_percent'] ?>%</p>
    </div>
    <div class="bg-white rounded-xl px-4 py-3 border border-gray-100 shadow-sm">
        <span class="text-xs text-gray-400 uppercase tracking-wide">Status</span>
        <p class="mt-0.5"><?= getStatusBadge($investor['status']) ?></p>
    </div>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                <?php
                $txns = $db->getdata("SELECT * FROM investor_ledger WHERE investor_id = $id AND type IN ('deposit','investment_withdraw') ORDER BY date ASC, id ASC");
                $bal = 0; $sl = 1;
                foreach ($txns as $t):
            ...
            <tfoot>
                <tr class="bg-gray-50 font-semibold text-sm">
                    ...
                    <td class="px-4 py-3 text-right text-rose-600"><?= showMoney($total_deposit) ?></td>
                    <td class="px-4 py-3 text-right text-emerald-600"><?= showMoney($total_inv_withdraw) ?></td>
...
function loadProfitLedger() {
    ...
    var to = $('#pf_to').val();
    $.get('<?= domain ?>includes/ajax_profit_ledger.php', { investor_id: <?= $id ?>, from_date: from, to_date: to }, function(data) {
